    ...
    while ((n = read(cli->socket, buffer, sizeof(buffer))) > 0) {
        ...
        if (strcmp(token, "REGISTER") == 0) {
            ...
            if (strcmp(token, "CHAT") == 0) {
                ...
                edit_channel(cli->logged_in_channel, new_channel, cli);
            } else if (strcmp(token, "ROOM") == 0) {
                ...
                bool is_password = strstr(new_value, "password") != NULL;
            ...
            token = strtok(NULL, " ");
            if (token == NULL) {
                char response[] = "Format perintah DELETE tidak valid";
                if (write(cli->socket, response, strlen(response)) < 0) {
                    perror("Gagal mengirim respons ke client");
                }
                continue;
            ...
            if (strcmp(token, "CHAT") == 0) {
                ...
                delete_channel(cli->logged_in_channel, cli);
            } else if (strcmp(token, "ROOM") == 0) {
                char *room = strtok(NULL, " ");
                delete_room(cli->logged_in_channel, room, cli);
            } else if (strcmp(token, "ALLROOMS") == 0) {
            ...
            char *target_user = strtok(NULL, " ");
            strstr(cli->logged_in_role, "ROOT") != NULL ? remove_user_root(target_user, cli) : remove_user(cli->logged_in_channel, target_user, cli);
...
void verify_key(const char *username, const char *channel, const char *key, client_info *client) {
    ...
    if (key_valid) {
        ...
        if (user_found) {
            char auth_path[256];
            snprintf(auth_path, sizeof(auth_path), "/home/ubuntu/FP/%s/admin/auth.csv", channel);
...
void send_chat(const char *username, const char *channel, const char *room, const char *message, client_info *client) {
    ...
    char path[256];
    snprintf(path, sizeof(path), "/home/ubuntu/FP/%s/%s/chat.csv", channel, room);
...
void edit_chat(const char *channel, const char *room, int id_chat, const char *new_text, client_info *client) {
    ...
    char temp_path[256];
    snprintf(temp_path, sizeof(temp_path), "/home/ubuntu/FP/%s/%s/chat_temp.csv", channel, room);
...
void ban_user(const char *channel, const char *target_user, client_info *client) {
    ...
    char temp_path[256];
    snprintf(temp_path, sizeof(temp_path), "/home/ubuntu/FP/%s/admin/auth_temp.csv", channel);
    FILE *temp_file = fopen(temp_path, "w+");
    if (!temp_file) {
